...
from ase.geometry import cellpar_to_cell, wrap_positions
...
def mof_cell(
    ...
    """
    Constructs a unit cell by replicating the ligand and metal centers to fully coordinate
    all the convex hull atoms of the metal centers with ligands, forming a 3D network.
...
def rotate_ligand_to_vector(ligand, target_vector, bonding_site_indices):
    """
    Rotates the ligand so that the vector from its centroid to the bonding site centroid aligns with the target vector.
    
    Parameters:
    - ligand: ASE Atoms object of the ligand.
    - target_vector: The vector to align the ligand with.
    - bonding_site_indices: Indices of the bonding site atoms in the ligand.
    
    Returns:
    - Rotated ligand as an ASE Atoms object.
    """
    ...
    bonding_site_positions = ligand_copy.get_positions()[bonding_site_indices]
    bonding_site_centroid = np.mean(bonding_site_positions, axis=0)
