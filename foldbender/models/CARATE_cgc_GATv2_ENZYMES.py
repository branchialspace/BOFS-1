...
path = os.path.join(os.getcwd(), 'data', 'ENZYMES')
...
def test(loader, epoch, test=False):
    ...
    if test:
        ...
        np.savetxt(f"ENZYMES_epoch{epoch}.csv", outputs)
...
for epoch in range(1, epochs + 1):
    ...
    x = np.loadtxt(f"ENZYMES_epoch{epoch}.csv")
    ...
    if store_auc/dataset.num_classes >= 0.9:
        break
